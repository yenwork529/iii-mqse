...
/**
 * From https://bitbucket.org/Airic/lunarutil/src/master/
 */
public final class LunarUtils {
     ...
     * 二十四節氣.
     */
    public static final String[] LunarHolDayName = {"小寒", "大寒", "立春", "雨水", "驚蟄", "春分",
     ...
     * @return the optional
     */
    public static Optional<Date> lunar2Date(int year, int month, int day, boolean isLeapMonth) {
        if (!checkDateInputValidRange(new Lunar(year, month, day, isLeapMonth))) {
        ...
        DateTime gregorianDate = date19000130.plusDays(totalDays);
        return Optional.of(gregorianDate.toDate());
    }
     ...
     * @param date
     */
    public static Optional<Lunar> date2Lunar(Date date) {
        if (!checkDateInputValidRange(date)) {
            return Optional.empty();
        }
        ...
        DateTime dateGregorian = new DateTime(year, month, day, 0, 0, 0, DateTimeZone.forID("UTC"));
        Duration duration = new Duration(date19000131, dateGregorian);
        ...
        /*
         * 求出了農曆年份。 農曆年份是以幹支標識的，以1900年1月31日己亥年開始。 默認1900為己亥年，1901則為辛丑。
    ...
    public static Optional<Lunar> getPrevLunar(int year, int month, int day, boolean isLeap) {
        Optional<Date> date = lunar2Date(year, month, day, isLeap);
        if (date.isPresent()) {
            Calendar c = Calendar.getInstance();
            c.setTime(date.get());
            c.add(Calendar.DATE, -1);
            return date2Lunar(c.getTime());
        }
        return Optional.empty();
    }
    ...
    public static String getLunarExpress(Lunar lunar) {
        return getLunarExpress(lunar, true, true, true);
    }
     ...
     * @return the string
     */
    public static String getLunarExpress(Lunar lunar, boolean outputYear, boolean outputMonth, boolean outputDay) {
        StringBuilder sb = new StringBuilder();
        if (outputYear) {
            sb.append(lunar.getYear()).append("(").append(cyclical(lunar.getYear())).append(")年");
        }
        if (outputMonth) {
            sb.append(lunar.isLeap() ? "閏" : "").append(getChnMonth(lunar.getMonth())).append("月");
        }
        if (outputDay) {
            sb.append(getChnDay(lunar.getDay()));
        }
        return sb.toString();
    }
    ...
    private static int leapDays(int yyyy) {
        if (leapMonth(yyyy) != 0) {
            if ((LUNAR_INFO[yyyy - 1900] & 0x10000) != 0) { return 30; } else { return 29; }
        } else { return 0; }
    }
    ...
    private static int monthDays(int y, int m) {
        if ((LUNAR_INFO[y - 1900] & (0x10000 >> m)) == 0) { return 29; } else { return 30; }
    }
    ...
    private static String getChinaDayString(int day) {
        ...
        if (day > 30) { return ""; }
        if (day == 10) { return "初十"; }
        if (day == 20) { return "二十"; } else { return chineseTen[day / 10] + CHN_NUMBER[n]; }
    }

    /**
     * Check date input valid range.
     *
     * @param lunar the lunar
     * @return true, if successful
     */
    private static boolean checkDateInputValidRange(Lunar lunar) {
     ...
     * @param date
     */
    private static boolean checkDateInputValidRange(Date date) {
        if (SOLAR_MIN_DATE.toDate().compareTo(date) == 1 || SOLAR_MAX_DATE.toDate().compareTo(date) == -1) {
            return false;
        }
