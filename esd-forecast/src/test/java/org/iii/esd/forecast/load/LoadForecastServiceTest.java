...
@SpringBootTest(classes = { LoadForecastService.class, ElectricDataService.class, FieldProfileService.class,
		WeatherService.class, UpdateService.class,TemperatureDataService.class,KwEstimationService.class })
@EnableAutoConfiguration
@Log4j2
public class LoadForecastServiceTest {

	@Autowired
	LoadForecastService service;
	...
	@Test
	@Order(1)
	void LoadDataTraning() {
		service.LoadDataTraning(profileId);
	}
	 ...
	 */
	@Test
	void CreateLoadKWEstimationResult() {
		
		@SuppressWarnings("deprecation")
		List<KwEstimation> result = service.CreateLoadKWEstimationResult(profileId, new Date("2017/05/03"));
		if(result.size()!=96) {
			fail();
		}
		result.forEach(a->{
	 ...
	 */
	@Test
	void CreateLoadKWEstimationResult2() {
		
		@SuppressWarnings("deprecation")
		List<KwEstimation> result = service.CreateLoadKWEstimationResult(profileId, new Date("2017/09/10"));
		if(result.size()!=96) {
			fail();
		}
		result.forEach(a->{
